...
const getOrderItems = async (orderId) => {
  ...
  for (const item of items) {
    ...
    item.meta = {};
    metaRows.forEach((r) => { item.meta[r.meta_key] = r.meta_value; });
  }
  return items;
};
...
const showOrder = async (req, res) => {
  try {
    ...
    order.payment_method     = meta["_payment_method_title"] || meta["_payment_method"] || "";
    ...
    order.shipping_first_name = meta["_shipping_first_name"] || order.billing_first_name;
    order.shipping_last_name  = meta["_shipping_last_name"]  || order.billing_last_name;
    order.shipping_address_1  = meta["_shipping_address_1"]  || order.billing_address_1;
    order.shipping_address_2  = meta["_shipping_address_2"]  || order.billing_address_2;
    order.shipping_city       = meta["_shipping_city"]       || order.billing_city;
    order.shipping_state      = meta["_shipping_state"]      || order.billing_state;
    order.shipping_postcode   = meta["_shipping_postcode"]   || order.billing_postcode;
    order.shipping_country    = meta["_shipping_country"]    || order.billing_country;

    const allItems       = await getOrderItems(id);
    order.line_items     = allItems.filter((i) => i.order_item_type === "line_item");
    order.shipping_items = allItems.filter((i) => i.order_item_type === "shipping");
    ...
    for (const item of order.line_items) {
      ...
      Object.entries(item.meta).forEach(([key, val]) => {
        if (key.startsWith("pa_") && val) {
          ...
          item.attributes.push(label.charAt(0).toUpperCase() + label.slice(1) + ": " + val);
        }
      });
    }
    ...
    const statuses = Object.entries(statusBadge).map(([value, info]) => ({
      value, label: info.label, class: info.class,
    }));

    res.render("orders/show", {
      title: "Order #" + id,
      order, refunds, customer, statuses, statusBadge,
      success: req.query.success || null,
      error:   req.query.error   || null,
    });
  } catch (err) {
...
const updateOrderStatus = async (req, res) => {
  try {
    ...
    res.redirect(`/store/admin/orders/${req.params.id}?error=` + encodeURIComponent(err.message));
  }
};
...
const deleteOrder = async (req, res) => {
  try {
    const { id } = req.params;
    const [items] = await db.query(
      "SELECT order_item_id FROM tbl_order_items WHERE order_id = ?", [id]
    );
    for (const item of items) {
      await db.query("DELETE FROM tbl_order_itemmeta WHERE order_item_id = ?", [item.order_item_id]);
    }
    ...
    res.redirect("/store/admin/orders?error=" + encodeURIComponent(err.message));
  }
};
...
const showAddOrder = async (req, res) => {
  try {
    ...
    res.render("orders/add", {
      title: "Add Order", customers, products,
...
const getProductVariations = async (req, res) => {
  try {
    ...
    for (const v of variations) {
      ...
      v.label = Object.values(v.attributes)
...
const storeOrder = async (req, res) => {
  ...
  try {
    ...
    const datePart = now.getFullYear() + "-" + months[now.getMonth()] + "-" + pad(now.getDate());
    const timePart = pad(now.getHours()) + pad(now.getMinutes()) + "-" + (now.getHours() < 12 ? "am" : "pm");
    ...
    const [orderResult] = await conn.query(
      ...
      [body.customer_id || 0, orderName, orderTitle, body.order_status || "wc-pending"]
    );
    ...
    await insertMeta("_payment_method_title", body.payment_method_title  || "Cash on delivery");
    ...
    await insertMeta("_billing_first_name",   body.billing_first_name    || "");
    await insertMeta("_billing_last_name",    body.billing_last_name     || "");
    await insertMeta("_billing_email",        body.billing_email         || "");
    await insertMeta("_billing_phone",        body.billing_phone         || "");
    await insertMeta("_billing_address_1",    body.billing_address_1     || "");
    await insertMeta("_billing_address_2",    body.billing_address_2     || "");
    await insertMeta("_billing_city",         body.billing_city          || "");
    await insertMeta("_billing_state",        body.billing_state         || "");
    await insertMeta("_billing_postcode",     body.billing_postcode      || "");
    await insertMeta("_billing_country",      body.billing_country       || "");
    await insertMeta("_shipping_first_name",  body.shipping_first_name   || body.billing_first_name  || "");
    await insertMeta("_shipping_last_name",   body.shipping_last_name    || body.billing_last_name   || "");
    await insertMeta("_shipping_address_1",   body.shipping_address_1    || body.billing_address_1   || "");
    await insertMeta("_shipping_address_2",   body.shipping_address_2    || body.billing_address_2   || "");
    await insertMeta("_shipping_city",        body.shipping_city         || body.billing_city        || "");
    await insertMeta("_shipping_state",       body.shipping_state        || body.billing_state       || "");
    await insertMeta("_shipping_postcode",    body.shipping_postcode     || body.billing_postcode    || "");
    await insertMeta("_shipping_country",     body.shipping_country      || body.billing_country     || "");
    await insertMeta("_shipping_phone",       body.shipping_phone        || body.billing_phone       || "");
    ...
    await conn.commit();
    res.redirect(`/store/admin/orders/${orderId}?success=Order created successfully`);
  } catch (err) {
    await conn.rollback();
    console.error("storeOrder error:", err.message, err.stack);
    res.redirect("/store/admin/orders/add?error=" + encodeURIComponent(err.message));
  } finally {
    conn.release();
  }
};
